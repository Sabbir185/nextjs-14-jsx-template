...
const FormInput = ({ label, name, placeholder, required, isEmail, initialValue, rules = [], textArea, type, readOnly, onChange }) => {
    ...
    let input = <input className={`form-input ${readOnly && "bg-gray-100"}`} type={type} onChange={onChange} placeholder={placeholder} readOnly={readOnly} />
    textArea && (input = <textarea className="form-input" placeholder={placeholder}/>)
